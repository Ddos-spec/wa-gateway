...
export const editUser = async (c) => {
    ...
    try {
        const result = await query("UPDATE users SET name = $1, email = $2, company_name = $3, plan_id = $4, billing_status = $5, status = $6, updated_at = CURRENT_TIMESTAMP WHERE id = $7 RETURNING id, name, email, company_name, billing_status, status", [name, email, company_name, plan_id, billing_status, status, id]);
...
export const suspendUser = async (c) => {
    const { id } = c.req.param();
    if (!id) {
        throw new HTTPException(400, { message: "User ID is required" });
    }
    try {
        const result = await query("UPDATE users SET status = 'suspended', updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id, status", [id]);
        if (result.rows.length === 0) {
            throw new HTTPException(404, { message: "User not found" });
        }
