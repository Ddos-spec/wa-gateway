            ...
            whatsapp.startSession(payload.session, {
              onConnected: async () => {
                ...
                try {
                  ...
                  const profileName = (session?.user as any)?.pushname || 'Unknown';
                  const waNumber = session?.user?.id?.split(':')[0] || 'Unknown';
                  await query(
                    'UPDATE sessions SET status = $1, profile_name = $2, wa_number = $3, updated_at = CURRENT_TIMESTAMP WHERE session_name = $4',
                    ['online', profileName, waNumber, payload.session]
                  );
